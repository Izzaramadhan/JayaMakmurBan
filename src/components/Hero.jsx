...
export default function Hero() {
  const whatsappLink = createWhatsAppLink(
    business.phoneWa,
    `Halo Admin ${business.shortName}, saya ingin tanya ketersediaan ban/velg dan booking servis.`
  );

  return (
    <section id="beranda" className="relative isolate overflow-hidden bg-slate-950 text-white">
      <div
      ...
      <div className="mx-auto grid min-h-[82vh] max-w-6xl items-center gap-10 px-4 py-24 md:grid-cols-[1.2fr_0.8fr]">
        <div>
          ...
          <h1 className="max-w-4xl text-4xl font-black leading-tight tracking-tight sm:text-5xl lg:text-6xl">
            Cek Ban, Pilih Jadwal, Servis Mobil Jadi Lebih Cepat.
          </h1>
          <p className="mt-6 max-w-2xl text-lg leading-8 text-slate-300">
            Website katalog dan booking online untuk membantu pelanggan Jaya Makmur Ban Klaten mengecek pilihan ban, velg, serta memesan layanan spooring, balancing, dan ganti ban tanpa antre lama.
          </p>
        ...
        <div className="rounded-3xl border border-white/10 bg-white/10 p-6 shadow-2xl backdrop-blur">
          <p className="text-sm font-semibold text-yellow-300">Fitur Utama Website</p>
          <div className="mt-5 space-y-4">
            {[
              "Katalog ban dan velg digital",
              "Booking spooring, balancing, dan ganti ban",
              "Kontak WhatsApp cepat",
              "Optimasi pencarian lokal wilayah Klaten",
            ].map((item) => (
              <div key={item} className="flex items-center gap-3 rounded-2xl bg-slate-950/50 p-4">
                <span className="flex h-8 w-8 items-center justify-center rounded-full bg-yellow-400 text-sm font-black text-slate-950">✓</span>
